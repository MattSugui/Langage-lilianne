...
#include <afxext.h>
#include "..\lilydevadv\lilydevadvDoc.h"
#include "..\lilydevadv\lilydevadvView.h"
...
protected:
	...
	DECLARE_DOCUMENT(ClilydevadvDoc)
...
public:
	virtual HRESULT InitializeDocumentPreview(HWND hWndParent, RECT* prc)
	{
		...
		CCreateContext ctx;
		ctx.m_pNewViewClass = RUNTIME_CLASS(ClilydevadvView);
		...
		m_pDocument->AddRef();
		ctx.m_pCurrentDoc = DYNAMIC_DOWNCAST(ClilydevadvDoc, (CObject*) m_pDocument->GetContainer());
